...
const ManagementPage = () => {
  ...
  const memoizedFiltering = useMemo(() => filtering, [filtering]);
  ...
  const { data, isSuccess } = useQuery({
    queryKey: ["me-post", value],
    queryFn: useMemo(
      () => async () => await postService.getAllByMe(pagination, value),
      [pagination, value]
    ),
  });
      ...
      cell: ({ row }) => {
        return (
          <Button
            ...
            color="warning"
            onClick={() =>
              navigate(`/management/posts/update/${row.getValue("id")}`, {
                replace: true,
              })
            }
          >
  ...
  const { getHeaderGroups, getRowModel } = useReactTable({
    ...
    state: { sorting, globalFilter: memoizedFiltering },
    onSortingChange: setSorting,
    onGlobalFilterChange: setFiltering,
    getFilteredRowModel: getFilteredRowModel(),
  });
  const memoizedRowModel = useMemo(() => getRowModel(), [getRowModel]);
